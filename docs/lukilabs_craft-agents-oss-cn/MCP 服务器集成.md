...
- 工具发现与代理
  ...
  - getProxyToolDefs：生成 mcp__{slug}__{toolName} 的代理工具定义
- 工具执行（callTool）
  - 解析代理名到源与原始工具名
  - 执行并统一处理文本/图片/音频内容块
  - 大响应保护与摘要回调
...
- 服务器实例
  ...
  - 停止时关闭传输、服务器与重置端口
- 名称规范化
  - 内部使用 mcp__{slug}__{toolName}，对外剥离 mcp__ 前缀，便于上层工具注册
...
- 会话工具过滤
  - 支持按执行模式、安全模式筛选工具名集合
- 与 MCP 的结合
  - 通过 getProxyToolDefs 将工具注册为 mcp__{slug}__{toolName}
  - 在后端或池化服务器中以统一命名消费
...
- 工具不可用
  - 代理工具名拼写错误：确保使用 mcp__{slug}__{toolName} 格式
