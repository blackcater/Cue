...
- 目录与文件策略
  ...
  - directories.buildResources：构建期资源根目录，配合 files 中的 dist/**/* 实现统一打包。
...
- macOS 平台配置
  ...
  - notarize：注释掉的可选步骤，需设置 APPLE_* 环境变量。
  - 参考路径：
...
### 资源处理、图标生成与许可证
- 资源目录与打包策略
  - resources/ 作为构建期资源根目录，通过 files 与 extraResources 精细控制包含与排除，确保 dist/**/* 能覆盖已复制的资源。
...
- 额外文件管理
  - Python 工具与平台包装器：resources/scripts/**/* 与 resources/bin/*（Unix/Windows 双端）通过 files 显式包含。
