...
- MCP 客户端池（McpClientPool）
  - 统一管理远端与本地 MCP 源，缓存工具清单，构建代理工具命名空间（mcp__{slug}__{toolName}），集中执行工具并处理二进制与大响应。
...
- 功能要点
  ...
  - 工具发现：缓存每个源的工具清单；生成代理工具定义（mcp__{slug}__{toolName}）。
...
- 工具不可见
  - 确认已通过 sync 正确注册源；检查代理工具命名是否符合 mcp__{slug}__{toolName}。
- 执行异常
